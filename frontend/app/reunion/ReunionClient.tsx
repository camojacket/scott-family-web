...
export interface ReunionSettings {
  reunion_number?: string;
  reunion_location?: string;
  reunion_hosted_by?: string;
  reunion_info_packet_url?: string;
}

export default function ReunionClient({ initialSettings }: { initialSettings?: ReunionSettings } = {}) {
  const { family } = useFamilyName();
  const [reunionNumber, setReunionNumber] = useState(initialSettings?.reunion_number ?? '94');
  ...
  // ── Load settings + RSVP ──
  useEffect(() => {
    // Only fetch settings if not provided by server
    if (!initialSettings) {
