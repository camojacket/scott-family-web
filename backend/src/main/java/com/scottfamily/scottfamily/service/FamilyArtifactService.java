...
@Service
public class FamilyArtifactService {
    ...
    public FamilyArtifactService(DSLContext dsl, CdnUploadService cdnUploadService, BlobContainerClient blobContainer) {
        this.dsl = dsl;
    ...
    public record FamilyArtifactDto(
            Long id,
            String name,
            String pdfUrl,
            String issueDate,
    ...
    public record CreateFamilyArtifactRequest(String name, String issueDate) {}

    public record UpdateFamilyArtifactRequest(String name, String issueDate) {}

    // ── Queries ──

    /** All family artifacts, sorted by issue_date DESC then name ASC */
    public List<FamilyArtifactDto> getAll() {
        return dsl.select(F_ID, F_NAME, F_PDF_URL, F_ISSUE_DATE, F_CREATED_AT, F_UPDATED_AT)
                .from(FAMILY_ARTIFACTS)
                .orderBy(F_ISSUE_DATE.desc(), F_NAME.asc())
                .fetch(this::toDto);
    }

    /** Create a family artifact record after PDF is uploaded */
    @Transactional
    public FamilyArtifactDto create(String name, String issueDate, String pdfUrl, String blobKey) {
        LocalDateTime now = LocalDateTime.now();
        Record r = dsl.insertInto(FAMILY_ARTIFACTS)
                .set(F_NAME, name)
    ...
    @Transactional
    public FamilyArtifactDto update(Long id, UpdateFamilyArtifactRequest req) {
        var stmt = dsl.update(FAMILY_ARTIFACTS).set(F_UPDATED_AT, LocalDateTime.now());
    ...
    @Transactional
    public FamilyArtifactDto reupload(Long id, String newPdfUrl, String newBlobKey) {
        String oldBlobKey = dsl.select(F_BLOB_KEY).from(FAMILY_ARTIFACTS)
                .where(F_ID.eq(id)).fetchOneInto(String.class);

        dsl.update(FAMILY_ARTIFACTS)
    ...
    /** Delete family artifact and its blob */
    @Transactional
    public void delete(Long id) {
        String blobKey = dsl.select(F_BLOB_KEY).from(FAMILY_ARTIFACTS)
                .where(F_ID.eq(id)).fetchOneInto(String.class);

        dsl.deleteFrom(FAMILY_ARTIFACTS).where(F_ID.eq(id)).execute();
    ...
    private FamilyArtifactDto toDto(Record r) {
        return new FamilyArtifactDto(
                r.get(F_ID),
                r.get(F_NAME),
                r.get(F_PDF_URL),
